...
from vbac_features import FEATURES_TO_USE, REPORTING_FLAGS

# Load data
# Download 2019 dataset from https://www.cdc.gov/nchs/data_access/vitalstatsonline.htm
data = open("data/2019_vbac_data.txt", "r")
parsed_data = open("data/2019_birth_data_parsed.csv", "w")
...
parsed_data = open("data/2019_birth_data_parsed.csv", "r")
vbac_only_data = open("data/2019_vbac_data.csv", "w")
...
for d_line in data_lines:
    ...
    if int(DELIVERY_METHOD_1) == 2 or int(DELIVERY_METHOD_2) == 1:
        ...
        int(DELIVERY_METHOD_1) == 4 or int(DELIVERY_METHOD_2) == 2
    ) and TOL_ATTEMPTED == "Y":
        count_no += 1
